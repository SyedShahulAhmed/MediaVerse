...
export default function Following() {
  ...
  useEffect(() => {
    ...
    const results = following.filter(
      (f) =>
  ...
  const handleUnfollow = async (targetId) => {
    try {
      ...
      await API.post(`/users/unfollow/${targetId}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });

      toast.success("👋 Unfollowed successfully!");
      setFollowing((prev) => prev.filter((f) => f._id !== targetId));
      setFiltered((prev) => prev.filter((f) => f._id !== targetId));
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0e] via-[#0d0d12] to-[#16161c] text-white px-6 py-12 relative overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto flex flex-col items-center justify-center mb-12 mt-4 text-center">
        ...
        <p className="text-gray-400 text-sm mt-2">
          {following.length} following account{following.length !== 1 && "s"}
        </p>
            ...
            {filtered.map((f, i) => {
              ...
              return (
                <div
                  ...
                  className="group relative bg-gradient-to-br from-[#191920]/80 to-[#221528]/70 border border-purple-700/40 rounded-2xl p-6 flex flex-col items-center text-center
                    hover:shadow-[0_0_45px_rgba(155,92,246,0.25)] hover:border-purple-400/50 hover:scale-[1.04]
                    transition-all duration-500 ease-out cursor-pointer overflow-hidden backdrop-blur-sm"
                >
                  {/* Glow Layer */}
                  ...
                  <button
                    ...
                    className="mt-5 text-xs bg-[#1b1b22]/90 hover:bg-[#252530] border border-purple-700/40 hover:border-fuchsia-500/50 px-3 py-1.5 rounded-lg flex items-center gap-1 text-gray-200 hover:text-white transition-all z-10"
                  >
                    <UserX size={12} /> Unfollow
                  </button>
                </div>
              );
            })}
